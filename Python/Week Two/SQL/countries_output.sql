...
FROM cities
WHERE population > 500000 AND cities.country_id = 136;

-- 4. What query would you run to get all languages in each country with a percentage greater than 89%? Your query should arrange the result by percentage in descending order. (1)

SELECT countries.name, languages.language, languages.percentage
FROM countries, languages
WHERE languages.percentage > 89 AND languages.country_code = countries.code
ORDER BY percentage DESC;
...
SELECT countries.region AS 'Region', COUNT(countries.id) AS 'Number of Countries'
FROM countries
